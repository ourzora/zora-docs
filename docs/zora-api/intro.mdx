...
[NFT Hooks](../developer-tools/nft-hooks/intro): A tool that makes NFT data retriveal even easier than the ZDK
...
| > 30 requests per minute   | Key Needed              |

Direct message [@ZORAEngineering](https://twitter.com/ZORAEngineering) on Twitter for an API Key.
Add a header in your requests with the key `X-API-KEY` to access the API with a key.
