...
Get started building with the Zora API

---
The Zora Development Kit (ZDK) makes it easy to access the Zora API without having to know GraphQL.
...
Please reach out to [@ZORAEngineering](https://twitter.com/ZORAEngineering) on Twitter if would like a key.

The max response size limit for normal a request is **50**.

[NFT Hooks](../developer-tools/nft-hooks/intro) is also available and makes it even easier to fetch NFT data than the ZDK.
:::
...
Also all `network` parameters are optional and will default to `Ethereum Mainnet`.
...
| network |  | the network to use to retrieve the token |
| network.chain | <code>Chain</code> | Chain to use (default ETHEREUM) |
| network.network | <code>Network</code> | Network on the given chain to use (default MAINNET) |
