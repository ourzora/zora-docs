...
This module provides read and write access to the Zora NFT contracts

---

Details about the ZORA smart contracts can be found [here](../../smart-contracts/zora-contracts). To instantiate the module, an ethers.js `Signer` or `Provider`, must be passed to the constructor.
As well as the `chainID`representing the Blockchain network you are trying to connect to. ZORA currently supports Ethereum Mainnet
(1) and Rinkeby Testnet (4).
...
All functions that are able to read data from the ZORA NFT smart contracts
...
All functions that are able to submit transactions to interact with the ZORA NFT smart contracts
