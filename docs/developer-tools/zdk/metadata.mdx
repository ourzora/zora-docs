...
In the original ERC-721 specification, it was suggested that tokens should link to a metadata URI that contains a link to the specific content of the token. 
...
ZORA NFTs explicitly separates content and metadata URIs and provides a `sha256` checksum for each. 
