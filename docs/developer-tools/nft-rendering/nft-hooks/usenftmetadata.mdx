...
Fetches Metadata information for zNFT (ZORA NFT)

---

This hook makes a request to fetch metadata from the zNFT's `metadataURI`.

Most metadata servers allow remote JSON fetches, including all ZORA NFTs. There is a chance this request could fail when the server
