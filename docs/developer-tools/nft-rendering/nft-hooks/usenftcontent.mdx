...
Fetches and loads the media content that is stored for a zNFT (ZORA NFT)

---

This hook makes a request to fetch content based on zNFT's `contentURI`.
...
To use the hook, simply pass in the `contentURI` from the zNFT on-chain data and `mimeType` (image, video, audio etc) from the zNFT's Metadata.
