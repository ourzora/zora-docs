...
The main types are within the `data.nft` object which contains the on-chain NFT data. 
The pricing information is in `data.pricing` which corresponds to data on-chain for ZORA's Auction House and the
...
const result = await fetchAgent.loadNFTData("0x8d04a8c79ceb0889bdd12acdf3fa9d207ed3ff63","2");
