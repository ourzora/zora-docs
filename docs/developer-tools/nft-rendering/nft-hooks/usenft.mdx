...
A hook that makes it easy to get data for any ERC-721 NFT

---

This hook fetches data for a given ERC-721 NFT and takes in the `contractAddress` and `tokenId`. 
...
The main types are within the result `data.nft` object. This object contains the on-chain NFT data itself. The pricing
information can be found in `data.pricing` which corresponds to data on-chain for ZORA's perpetual zNFT auctions along with the auction house functionality.
