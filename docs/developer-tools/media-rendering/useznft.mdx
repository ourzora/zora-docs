...
Loads zNFT data (Zora NFT) and market on-chain information

---

This hook fetches data for a given zNFT (Zora NFT). The only argument for the hook is the zNFT's `tokenId`. 

To fetch data for zNFTs on other networks, use the `NFTFetchConfiguration` wrapper component to set the correct network for loading the
