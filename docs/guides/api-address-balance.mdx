...
We can see that it has sold 15 NFTs for a total of 24 ETH.
<Tabs
  defaultValue="query"
  values={[
    { label: 'Query', value: 'query', },
...
## Largest Purchase
Lastly, we can get the largest on-chain ETH purchase this address has made using the `sales` query and sorting by descending `ETH_PRICE`.
