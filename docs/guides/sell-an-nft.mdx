...
import mainnetZoraAddresses from "@zoralabs/v3/dist/addresses/1.json"; // Mainnet addresses
...
This amount can be awarded to the party that helped match the buyer to the NFT.
...
If the NFT is priced in an ERC-20 token then the user trying to fill in the `ask` must approve their tokens to the [ERC-20 Transfer Helper](/docs/guides/v3-approvals#approving-a-transfer-helper) 
...
##### The Finder Address 
The `finder` is an address of the party that helped match the buyer to the NFT and is awarded the Finders Fee. 
