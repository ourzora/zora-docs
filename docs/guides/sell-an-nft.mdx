...
The `finder` address can be set to the `0 address` if no address is provided or no Finders Fee is set. 
...
It is always a best to directly cancel an `ask` rather than transfer the NFT to a different address.
```
