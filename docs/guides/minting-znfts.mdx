...
A guide on how to mint your first NFT on ZORA
...
For more info visit [Metadata](../developer-tools/zdk/metadata#generate-metadata).

### tokenURI

The uri where the zNFT's content is hosted externally to the blockchain.
ZORA strongly recommends using a _decentralized_ storage provider such as [IPFS](https://ipfs.io/) or arweave. The contentHash field is a sha-256
...
The uri where the zNFT's content is hosted.
ZORA strongly recommends using a _decentralized_ storage provider such as [IPFS](https://ipfs.io/) or arweave. The contentHash field is a sha-256
hash of the content of the content file. It is imperative that this hash is correct, because once it is written to the
blockchain it **cannot** be changed.
...
The immutable, perpetual equity (%) the creator gets from each accepted bid of the zNFT.

### owner

The equity (%) the current owner gets from the next accepted bid of the zNFT.

### prevOwner

The equity (%) the previous owner gets from the next accepted bid of the zNFT.
