...
##### ERC-721 Tokens
ZORA V3 requires that the ERC-721 Transfer Helper has been approved as an operator in order to be able to move the NFT.
...
For example, if a user would like to sell an NFT via the `Asks v1.1 Module` they will need to approve that module in the Module Manage in order for the `Asks 1.1 Module` to be able to transfer the NFT in the event of a sale.
...
##### Multiple Modules
In addition, a user can batch approve modules in single transaction by calling `setBatchApprovalForModules` and passing in an array of module addresses with a boolean.
