...
The best way to find the mint fee for a specific contract is to retieve it from the contract by calling the `mintFee` function.
...
To mint a token the `mint` function is called on the main 1155 contract and then the execution is passed to a minter.

Note, that the minters need to be assigned the minter role to be able to mint and sell NFTs.
View the [permissions](./Permissions1155) section for more details.
...
`minterArguments`: Address to mint to
