---
title: Smart Contracts High-Level Overview
---

# ZORA Smart Contracts High-Level Overview

## Zora 1155 Contracts

### Creating a New Token

A ZORA contract create is when a new mint becomes available on the contract or when a contract is deployed and a new mint is created on the contract.

Now that we have both gasless and on-chain minting the mint event arguments are slightly different to determine the originating user of the mint.
...
The `CreatorAttribution` event is emitted only when bringing a token onchain. The creator of the token in this case is the `creator` field of the event.
...
This allows us to have the gas savings and security of a pull contract while also making it easy for users to retrieve all their rewards in one place.
