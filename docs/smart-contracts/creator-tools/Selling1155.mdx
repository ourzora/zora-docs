...
To sell a token, the minter strategy contract address must be given the minter role in the 1155 contract. The minter role can either be set at the contract level or on a per token basis. 
Setting the minter address as a minter role for tokenId 0 will give it contract-level minting privileges.
View the [permissions](./Permissions1155) section for more details on minting 
...
function callSale(
    uint256 tokenId,
    IMinter1155 salesConfig, // Minter Strategy Contract Address
    bytes memory data
) 
```
