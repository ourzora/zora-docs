...
`ERC-721 token address => ERC-721 token ID => Offer ID => Offer`
```
...
`ERC-721 token address => ERC-721 token ID => Offer IDs` 
```
...
- ZORA takes custory of the amount until either the `offer` is filled or cancelled
- The user must first approve the ERC-20 Transfer Helper to move their assets if the `offer` is created with an ERC-20 token
- No approvals are needed if the currency of the `offer` is ETH
```
...
- Decreasing the amount will refunded the delta back to the creator address
- Updating the currency to be an ERC-20 will require that the user first approves the ERC-20 Transfer Helper
```
