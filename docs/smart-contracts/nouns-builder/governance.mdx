...
- If the proposer owns enough NFTs  to clear the Proposal Threshold it is then put to a vote
- However, the proposal can't be voted on until after the voting delay time has passed
- Once the delay has concluded, the proposal must then receive more **For** votes than **Against** and the number of **For** votes must surpass the Quorum Threshold
