...
- `Time Lock Delay:` The amount of time a successful proposal must wait between being queued and exectuted.
...
- Finally, once the time lock has ended the the proposal is then executed.
