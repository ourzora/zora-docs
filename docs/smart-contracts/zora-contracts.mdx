...
NFTs minted from this contract are known as `zNFTs` (aka cryptomedia) and have a few addtional characterists that make them different from regular ERC-721s.

##### ZORA NFTs vs Regular NFTs
- zNFTs separate the content from the metadata by having both a `metadataURI` and a `contentURI`
- zNFTs store a hash of both the metadata and content
- The URI linking to external storage can be updated, but a zNFT's metadata and content hashes can not
- Both the metadata and content storage location can be updated without having to worry about the integrity of the data

ZORA NFTs are composed of two contracts. 
The `Media` contract mints and stores token data. The `Market` contract allows for built-in market functionality for zNFTs. 
